...
# debug function
def print_pixels(pixels, borders):
    smallest_r = min(pixels.keys(), key=lambda p: p[0])[0]
    largest_r = max(pixels.keys(), key=lambda p: p[0])[0]
    smallest_c = min(pixels.keys(), key=lambda p: p[1])[1]
    largest_c = max(pixels.keys(), key=lambda p: p[1])[1]
    string = ""
    for r in range(smallest_r, largest_r + 1):
        for c in range(smallest_c, largest_c + 1):
            string += "#" if pixels[(r, c)] else "."
        string += "\n"
    print(string)


image_pixels, borders = enhance(image_pixels, 0)
